...
def interpolate_keypoints(keypoints, target_length=15):
    ...
    # Verificar que todos los keypoints tienen la longitud esperada
    for k in keypoints:
        if len(k) != LENGTH_KEYPOINTS:
            print(f"Keypoint length mismatch: Expected {LENGTH_KEYPOINTS}, but found {len(k)}")
            raise ValueError(f"Keypoint length mismatch: Expected {LENGTH_KEYPOINTS}, but found {len(k)}")
    ...
    for i in indices:
        ...
        if lower_idx == upper_idx:
            ...
            # Verificar que los keypoints tienen la misma longitud para la interpolación
            if len(keypoints[lower_idx]) != len(keypoints[upper_idx]):
                print(f"Keypoint length mismatch between indices {lower_idx} and {upper_idx}: {len(keypoints[lower_idx])} vs {len(keypoints[upper_idx])}")
                raise ValueError(f"Keypoint length mismatch between indices {lower_idx} and {upper_idx}")
